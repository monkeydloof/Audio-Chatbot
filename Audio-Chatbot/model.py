...
class Model():
    ...
    def _build_model(self):
        # 1D convolution
        self.conv1d_index = 0
        out = self.conv1d_layer(self.input_data, dim=self.n_dim)
        ...
        for _ in range(n_blocks):
            for r in [1, 2, 4, 8, 16]:
                out, s = self.residual_block(out, size=7, rate=r, dim=self.n_dim)
                skip += s

        logit = self.conv1d_layer(skip, dim=skip.get_shape().as_list()[-1])
        self.logit = self.conv1d_layer(logit, dim=self.n_out, bias=True, activation=None)
